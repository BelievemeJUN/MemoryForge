...
class KnowledgeBaseManager:
    ...
    def __init__(self, client: AsyncMilvusClient, embeddings: HuggingFaceBgeEmbeddings,
    ...
    async def init_collection(self):
        ...
        index_params.add_index(
            ...
            index_type="INVERTED",  # Milvus2.5 适配: STL_SORT 仅支持数值字段
        )

        index_params.add_index(
            field_name="file_hash",
            index_name="file_hash_idx",
            index_type="INVERTED",  # Milvus2.5 适配
        )
    ...
    async def hybrid_retrieval_knowledge_base(
        ...
        # 图上下文外调用时 writer 不可用（如评测/脚本直调）→ 降级为无操作
        try:
