...
async def test_low_threshold_with_shift_updates_profile(monkeypatch):
    """未达阈值 + 显式转折 → 提炼画像并 merge 进 PG。

    架构决策：稳定偏好唯一归 user_profile，不再写回 semantic 库（避免自造重复）——
    因此本路径不应触发任何 Milvus 调用。
    """
    ...
    async def _boom(*a, **k):
        raise AssertionError("偏好只归画像，不应触发 Milvus 写回")

    monkeypatch.setattr(m, "get_milvus_client", _boom)  # 若被调用会失败
